...
<div class="content-wrapper">
  <div class="container-xxl flex-grow-1 container-p-y">
    <div class="mb-4">
      <a href="{{ route('pisteurs.index') }}" class="text-primary">
        <i class="bx bx-arrow-back me-1"></i> Retour aux pisteurs
      </a>
    </div>

    <div class="d-flex align-items-center mb-2">
      <i class="bx bx-leaf fs-3 me-2 text-success"></i>
      <h4 class="mb-0">{{ $pisteur->nom }} {{ $pisteur->prenoms }}</h4>
    </div>
    <p class="text-muted mb-4">Contact: {{ $pisteur->contact ?? 'Non renseigné' }}</p>
    ...
    <div class="row">
      <div class="col-md-5">
        <div class="card mb-4">
          ...
          <div class="card-body">
            <p class="mb-2"><small class="text-muted">Nom</small><br><strong>{{ $pisteur->nom }}</strong></p>
            <p class="mb-2"><small class="text-muted">Prénoms</small><br><strong>{{ $pisteur->prenoms }}</strong></p>
            <p class="mb-2"><small class="text-muted">Contact</small><br><strong>{{ $pisteur->contact ?? '-' }}</strong></p>
            <p class="mb-0"><small class="text-muted">Date d'ajout</small><br><strong>{{ $pisteur->created_at->format('d-m-Y') }}</strong></p>
          </div>
        </div>
      </div>
          ...
          <div class="card-body p-0">
            <div class="table-responsive">
              <table class="table mb-0">
                ...
                <tbody>
                  @forelse($pisteur->prixPeriodes as $prix)
                    <tr>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header bg-success text-white">
        <h5 class="modal-title text-white"><i class="bx bx-plus-circle me-2"></i>Ajouter un prix par période</h5>
        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <form action="{{ route('pisteurs.prix.store', $pisteur) }}" method="POST">
        @csrf
        <div class="modal-body">
          <div class="mb-3">
            <label class="form-label">Prix unitaire (FCFA/tonne) <span class="text-danger">*</span></label>
...
@foreach($pisteur->prixPeriodes as $prix)
<!-- Modal Éditer Prix -->
<div class="modal fade" id="modalEditPrix{{ $prix->id }}" tabindex="-1" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header bg-warning">
        <h5 class="modal-title"><i class="bx bx-edit me-2"></i>Modifier le prix</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <form action="{{ route('pisteurs.prix.update', [$pisteur, $prix]) }}" method="POST">
        @csrf
        @method('PUT')
        <div class="modal-body">
          <div class="mb-3">
...
<div class="modal fade" id="modalDeletePrix{{ $prix->id }}" tabindex="-1" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        <button type="button" class="btn btn-outline-secondary" data-bs-dismiss="modal">Annuler</button>
        <form action="{{ route('pisteurs.prix.destroy', [$pisteur, $prix]) }}" method="POST" class="d-inline">
          @csrf
          @method('DELETE')
