...
<div class="content-wrapper">
  <div class="container-xxl flex-grow-1 container-p-y">
    <h4 class="fw-bold py-3 mb-4">
      <span class="text-muted fw-light">Gestion Camions PGF /</span> Revenues camions PGF
    </h4>
        ...
        <table class="table table-hover">
          ...
          <tbody>
            @forelse($data as $item)
              <tr>
                <td>
                  <a href="{{ route('camions.revenues.show') }}" class="text-primary fw-bold">
                    {{ $item['nom'] }}{{ $item['prenoms'] !== '' ? ' '.$item['prenoms'] : '' }}
                  </a>
                </td>
                <td>
                  <span class="badge bg-label-primary">{{ $item['code'] }}</span>
                </td>
                <td class="text-center">{{ $item['camions_count'] }}</td>
                <td class="text-end">
                ...
                <td class="text-center">
                  <a href="{{ route('camions.revenues.show') }}" class="btn btn-sm btn-outline-primary" title="Voir le détail">
                    <i class="bx bx-show"></i>
                  </a>
                  <a href="{{ route('camions.camions_pgf') }}" class="btn btn-sm btn-outline-secondary" title="Camions PGF">
            ...
            @empty
              <tr>
                <td colspan="7" class="text-center text-muted py-4">Aucun camion PGF enregistré</td>
              </tr>
            @endforelse
          </tbody>
          @if(count($data) > 0)
